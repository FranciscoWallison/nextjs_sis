...
class HelpQuestions {
  ...
  //     const itensFiltrados = categoria.filter((item) => {
  //       return (
  ...
  static filterItems = async (
    dataItens: Category[],
    itemsToFilter: Category[]
  ): Promise<Category[]> => {
    // Obter todos os IDs de atividades que precisam ser removidas
    const itemIdsToRemove = itemsToFilter.map((item) => item.id);

    // Filtrar as atividades em dataItens
    const filteredDataItens = dataItens.filter(
      (activity) => !itemIdsToRemove.includes(activity.id)
    );
  ...
  static removeActivityById = async (data: any[], activityId: any) => {
    return data.filter((activity) => activity.id !== activityId);
  };
}
